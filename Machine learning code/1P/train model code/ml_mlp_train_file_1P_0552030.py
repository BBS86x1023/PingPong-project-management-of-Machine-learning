...
#標籤(輸出結果)
data = np.hstack((Ballarray,PlatformPosition_1P,PlatformPosition_2P,Ball_Vx,Ball_Vy,instruct))
##-------------------------------------------------------------------------------------------------------##
...
for i in range(len(data)):
    buffer_data_y.append(data[i][8])
    buffer_data_x.append(data[i][0:8])
...
## 第一層有100個神經元以及8組特徵，激勵函數使用 X>0。 (輸入層)
model.add(Neurons(units=100, activation=Relu(), input_dim=train_data_x.shape[1]))
## 第二層有80個神經元，激勵函數使用 X>0。
model.add(Neurons(units=80, activation=Relu()))
## 第三層有50個神經元，激勵函數使用 X>0。
model.add(Neurons(units=50, activation=Relu()))
## 第四層有15個神經元，激勵函數使用 X>0。
model.add(Neurons(units=15, activation=Relu()))
## 第五層有5個神經元，激勵函數使用 X>0。
model.add(Neurons(units=5, activation=Relu()))
...
params = {"learning_rate": 0.0001, "n_epoch": 2500, "print_rate": 100}
## MSE : 均方誤差
loss = MSE()
## 印出多層神經網路架構
print(model.layers)
...
\\game\\MLGame-master\\games\\pingpong\\ml\\mlp_model_1P_0552030_1.sav"
